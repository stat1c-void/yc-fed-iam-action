...
[Workload Identity Federation](https://yandex.cloud/en/docs/iam/operations/wlif/setup-wlif). This
allows workflows to perform actions on Yandex Cloud resources on behalf of service accounts without
  ...
  example-job:
    ...
    steps:
      - name: Get Yandex IAM token
        id: ya_iam_token
        uses:
          # use full version tag or pin hash
          # releases 1.0.5 and later are immutable
          stat1c-void/yc-fed-iam-action@v1.0.5
        with:
...
If you are getting an error like `Unable to get ACTIONS_ID_TOKEN_REQUEST_URL env variable`, check
